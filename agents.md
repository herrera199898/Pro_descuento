...
- `scan_scope` usa `complete` por defecto tanto en API como en las interfaces global y de supermercados.
...
- VTEX/SMU: `jumbo`, `santaisabel`, `unimarc`, `alvi`. El scraper intenta API VTEX clasica y tiene fallback defensivo por HTML embebido cuando el sitio no expone JSON directo. Jumbo usa `https://bff.jumbo.cl/catalog/plp`; Santa Isabel expone `https://bff.santaisabel.cl/catalog/plp` en navegador, pero puede responder 401 desde requests sin token/sesion.
- Jumbo usa una lista curada de categorias raiz obtenida desde su menu principal. No exponer como categorias los enlaces promocionales o productos (`/p`) encontrados en el homepage.
- Santa Isabel usa el mismo BFF Cencosud, pero requiere el header estatico `apikey`, configurado mediante `SANTAISABEL_BFF_AUTH`. El local/store se configura mediante `SANTAISABEL_STORE` y usa `pedrofontova` por defecto. Las cookies del navegador no son necesarias. El body usa `selectedFacets: [{"key": "categoryN", "value": "/ruta-canonica"}]`, sin guion entre `category` y la profundidad.
- Alvi obtiene el arbol real desde `https://bff-alvi-web.alvi.cl/categories/`, conserva el `id` numerico y el `fq` jerarquico de cada slug, y consulta `https://bff-alvi-web.alvi.cl/products/by-category/` con el `fq` exacto. El parametro `category` de `products/intelligence-search-plp` es ignorado; ese endpoint filtra slugs con `categories=<slug/ruta>` y se usa solo como fallback si no se puede resolver el `fq`.
- Las busquedas por texto de Alvi usan `https://bff-alvi-web.alvi.cl/products/intelligence-search/<termino>/`; `query=<termino>` en `intelligence-search-plp` tambien es ignorado.
- Unimarc usa `POST https://bff-unimarc-ecommerce.unimarc.cl/catalog/product/search` con headers `source: web`, `channel: UNIMARC`, `version: 1.0.0`. El arbol real viene de `GET https://bff-unimarc-web.unimarc.cl/bff-api/categories/2`.
- Walmart: `lider`, `acuenta`. Lider requiere configurar `LIDER_API_TEMPLATE`. aCuenta usa Playwright sobre sus paginas Next.js SSR: busqueda en `/search?name=<termino>` y categorias reales Instaleap en `/ca/<slug>/<referencia>`.
- Falabella: `tottus`. Usa categorias reales curadas con IDs/rutas `CATG...`, por ejemplo `CATG27055/Despensa`. La categoria virtual `__offers__` agrega en sesiones aisladas todas las colecciones enlazadas desde `/tottus-cl/content/ofertas-tottus` y conserva solo productos con precio original tachado/descuento real. Ofertas ignora el limite de busqueda rapida para priorizar entregar todas las colecciones disponibles. Si existe `TOTTUS_API_TEMPLATE`, consulta ese endpoint JSON; de lo contrario abre la PLP con Playwright headless y lee los primeros 48 productos SSR desde `__NEXT_DATA__` (el SSR ignora `?page=2`). Nunca debe abrir un navegador visible; opcionalmente `TOTTUS_BROWSER_CHANNEL` permite seleccionar un canal instalado, pero por defecto usa Chromium de Playwright. Las busquedas generales usan la ruta oficial `/tottus-cl/buscar?Ntt=<termino>` y validan localmente los resultados para descartar el set por defecto que Tottus devuelve cuando no encuentra coincidencias. Si `/buscar` es bloqueado, sondea hasta tres categorias relevantes con un presupuesto total acotado y deja un warning en `meta`.
